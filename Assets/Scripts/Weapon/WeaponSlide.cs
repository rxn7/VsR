...
public class WeaponSlide : XRBaseInteractable {
	...
	private void UpdateSlideMovement() {
		...
		if (m_cocked && slidePercentage < 0.6f) {
			SoundManager.PlaySound(m_weapon.Data.cockBackSound, transform.position, Random.Range(0.9f, 1.1f));
			m_cocked = false;
		}

		if (!m_cocked && slidePercentage >= 0.99f) {
			SoundManager.PlaySound(m_weapon.Data.cockSound, transform.position, Random.Range(0.9f, 1.1f));
			m_weapon.Cock();
			m_cocked = true;
		}
	}
