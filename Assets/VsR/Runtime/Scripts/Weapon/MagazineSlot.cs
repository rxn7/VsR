...
namespace VsR {
	public class MagazineSlot : XRSocketInteractor {
		...
		private void OnMagazineSelected(Magazine mag) {
			...
			SoundManager.Instance.PlaySound(m_magazine.Data.slideInSound, transform.position, Random.Range(0.95f, 1.05f));
		...
		public void ReleaseMagazine() {
			...
			SoundManager.Instance.PlaySound(m_magazine.Data.slideOutSound, transform.position, Random.Range(0.95f, 1.05f));
